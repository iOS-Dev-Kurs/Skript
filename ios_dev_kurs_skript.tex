...
\item Öffnet in Xcode den \strong{Devices} Bereich mit \keys{\cmd + \shift + 2} und wählt in der linken Seitenleiste euer Gerät aus \abbref{img:xcode_device}.
...
Während wir an unserem Projekt arbeiten, können wir Git verwenden, um Versionen unseres Codes möglichst häufig in Form von \strong{Commits} zu sichern. Dabei werden alle Änderungen an den Projektdateien, die seit dem letzten Commit durchgeführt wurden, in einem versteckten \shinline{.git/} Verzeichnis, dem \strong{Repository}, hinterlegt.
...
Git bietet weiterhin die sehr nützliche Möglichkeit, verschiedene Versionen eines Projekts zu verwalten, indem sich die Commitfolge an einer beliebigen Stelle verzweigt. Dazu können wir mit \shinline{git branch} einen neuen \strong{Branch} erstellen. Es kann jederzeit mit \shinline{git checkout} zwischen Branches gewechselt werden. Dabei passt Git die Dateien und deren Inhalt im Repository automatisch an.
...
In dieser Form werden bspw. sehr häufig \strong{Feature Branches} verwaltet. Dabei wird die stabile und häufig veröffentlichte Version des Projekts von dem \shinline{master} Branch des Repositories repräsentiert. Für neue Features oder Umstrukturierungen wird dann eine Branchstruktur angelegt. So kann gearbeitet werden, ohne dass die stabile Version des Projekts verändert wird. Erreicht ein Branch einen stabilen Status und soll veröffentlicht werden, wird ein Merge mit dem \shinline{master} Branch durchgeführt. Der Arbeitsbranch kann dabei jederzeit gewechselt werden. So kann bspw. schnell ein Fehler in der veröffentlichten Version in \shinline{master} behoben und anschließend wieder zum Feature Branch gewechselt werden.
...
Ein auf GitHub erstelltes Repository kann mit \shinline{git clone} heruntergeladen werden, wobei den local Branches automatisch ihr entsprechendes remote Gegenstück zugewiesen wird. Anschließend können wir mit dem local Repository arbeiten und Commits mit dem remote Repository abgleichen.
...
Elemente können von von der Erfassung durch Git ausgenommen werden, wenn es sich bspw. um benutzerspezifische oder temporäre Dateien handelt. Dazu wird dem Repository eine \shinline{.gitignore} Datei hinzugefügt.
...
\item Die Methode \objcinline{main()} in der Datei \emph{main.m} wird aufgerufen. Diese sieht typischerweise wie folgt aus und braucht selten verändert zu werden:
...
Die \objcinline{main()} Methode wird auch als \strong{Entry Point} bezeichnet und besitzt entsprechende Äquivalente in \shinline{C} und \shinline{C++}.

\item \objcinline{main()} ruft \objcinline{UIApplicationMain()} auf. Mit einem \keys{\Alt}-Klick auf den Methodennamen können wir die Dokumentaion dieser Methode in Apple's \objcinline{UIKit} Framework anschauen \abbref{img:lifecycle_uiapplicationmain}.
...
\item Diese Methode erzeugt das \strong{Application Object}, also ein Objekt des Typs \objcinline{UIApplication}, das für die zentrale Koordination der App verantwortlich ist.

U.a. stellt dieses Objekt die Verbindung zum Bildschirm her, erzeugt ein \objcinline{UIWindow} zur Darstellung der Benutzeroberflöche und empfängt Benutzereingaben. Wir müssen für diese Klasse keinen weiteren Code schreiben, damit die Architektur der App funktioniert. Über den Aufruf einer Klassenmethode nach dem sog. \emph{Singleton Pattern} \secref{sec:singleton} kann jederzeit auf das \objcinline{UIApplication} Objekt zugegriffen werden:
...
Dabei wird die Klasse verwendet, die der \objcinline{UIApplicationMain()} Methode in Form eines \objcinline{NSString} Objekts übergeben wird:
\begin{objccode}
NSStringFromClass([XYZAppDelegate class]) // entspricht dem NSString @"XYZAppDelegate"
\end{objccode}
Das erstellte Objekt dieser Klasse wird der Property \objcinline{delegate} des Application Objects zugewiesen und ist daher immer verfügbar:
\begin{objccode}
[UIApplication sharedApplication].delegate // gibt das App Delegate Objekt zurück
\end{objccode}

Wir implementieren die Klasse \objcinline{XYZAppDelegate} wie jede andere Klasse in unserem Code mit ihrer Header- und Main-Datei.
...
Dieses Storyboard wird geladen und dessen Inital Scene angezeigt. Die im Storyboard enthaltenen Objekte werden dabei instanziert und die Connections (\objcinline{IBOutlets} und \objcinline{IBActions}) hergestellt.
...
Wir implementieren häufig Subklassen von \objcinline{NSObject} mit Eigenschaften und Klassenbeziehungen, um solche abstrakten Datenstrukturen zu repräsentieren.

\item[Präsentation (View)] Subklassen von \objcinline{UIView} repräsentieren Interfaceelemente und werden auf dem Bildschirm dargestellt. Sie werden vom Controller mit Informationen gefüllt und leiten Benutzereingaben an diesen weiter. 

\item[Steuerung (Controller)] Der Controller verwaltet die Views und reagiert auf Benutzereingaben. Zur Laufzeit der App übernehmen Subklassen von \objcinline{UIViewController} die Kommunikation zwischen Model und View. Meist repräsentiert jeweils ein View Controller eine bestimmte Ansicht auf dem Bildschirm. In Reaktion auf Benutzereingaben stellt der View Controller Anfragen an das Model und konfiguriert die Views.

Zusätzlich sind übergeordnete Controller wie bspw. Instanzen von \objcinline{UINavigationController} für die Koordination der einzelnen View Controller zuständig und verwalten deren hierarchische Strukturen.
\end{description}
...
Die \strong{View} Komponente jeder iOS App ist für die Anzeige des User Interface auf dem Bildschirm verantwortlich. Wir verwenden dazu Subklassen von \objcinline{UIView}.

\objcinline{UIView} wird in Apples UIKit Framework als Subklasse von \objcinline{UIResponder : NSObject} implementiert. Die Klasse erbt somit zusätzlich zu den Verwaltungsmechanismen von \objcinline{NSObject} auch die Methoden zur Reaktion auf Benutzereingaben von \objcinline{UIResponder}.

UIKit stellt viele Subklassen von \objcinline{UIView} zur Verfügung, wie bspw. \objcinline{UILabel}, \objcinline{UIButton} und \objcinline{UIImageView}. Diese implementieren jeweils Methoden, um ihren Inhalt auf dem Bildschirm zu rendern.

Jedes \objcinline{UIView} Objekt präsentiert jedoch nicht nur seinen eigenen Inhalt sondern dient auch als Container für andere \objcinline{UIView} Objekte. Somit erhalten wir eine hierarchische Struktur von \strong{Superviews} mit jeweils beliebig vielen \strong{Subviews} \abbref{img:view_hierarchy}. An oberster Stelle der Hierarchie steht dabei ein Objekt der Klasse \objcinline{UIWindow : UIView}, das von dem Application Object verwaltet wird \secref{sec:launchprocess}.

\includegraphicsc[\iphonewidth]{img/view_hierarchy.png}{img:view_hierarchy}{Jedes Objekt der \objcinline{UIView} Klasse dient wieder als Container für weitere Objekte dieser Klasse}

Mit Instanzmethoden wie \objcinline{addSubview:} und \objcinline{removeFromSuperview} von \objcinline{UIView} können wir der View Hierarchie Objekte hinzufügen oder Objekte entfernen.

\subsection{Frame und CGRect}

Jedes Objekt der Klasse \objcinline{UIView} verwaltet einen Anzeigebereich, der durch das Attribut \objcinline{CGRect frame} bestimmt ist. Der Frame bestimmt ein Rechteck im zweidimensionalen Koordinatensystem der Superview.
...
\includegraphicsc[\iphonewidth]{img/view_coordinates.png}{img:view_coordinates}{Der Ursprung des Koordinatensystems von \objcinline{UIView} liegt in der oberen linken Ecke}

\objcinline{CGRect} ist keine Klasse sondern ein \strong{Struct}, das ein Rechteck mit einem Ursprung \objcinline{CGPoint origin} und einer Größe \objcinline{CGSize size} repräsentiert. \objcinline{CGPoint} stellt dabei einen Punkt mit den Attributen \objcinline{CGFloat x} und \objcinline{CGFloat y} dar, während \objcinline{CGSize} eine Ausdehnung mit Breite \objcinline{CGFloat width} und Höhe \objcinline{CGFloat height} beschreibt. \objcinline{CGFloat} ist äquivalent zu \objcinline{float} auf einer 32-bit Architektur und zu \objcinline{double} auf einer 64-bit Architektur.
\begin{objccode}
struct CGRect {
   CGPoint origin;
   CGSize size;
...
Zur Erstellung eines \objcinline{CGRect} kann die Funktion \objcinline{CGRectMake(x, y, width, height)} verwendet werden.

\subsection{UIView Objekte}

Wir können ein \objcinline{UIView} Objekt u.a. mit der \objcinline{initWithFrame:} Methode erstellen und dann der View Hierarchie hinzufügen, sodass es auf dem Bildschirm angezeigt wird:
\begin{objccode}
UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 44)];
[self.view addSubview:view]; // Angenommen self.view ist bereits Teil der View Hierarchie
\end{objccode}

Subklassen von UIView erben diesen Mechanismus. Ein \objcinline{UILabel} lässt sich also bspw. genauso erzeugen und anzeigen.
...
Natürlich können wir versuchen, die Parameter des \objcinline{frame} Attributs einer View in Reaktion auf Änderungen der Größe der Superview oder des Inhalts geschickt anzupassen. Häufig haben wir jedoch ganz bestimmte Vorstellungen, wie die Views einer View Hierarchie \strong{zueinander} positioniert sein sollen. Bei der Konzeption von Benutzeroberflächen treten anstatt von festen Positionen vielmehr Regeln auf wie:
\begin{itemize}
...
Jede Constraint ist ein Objekt der Klasse \objcinline{NSLayoutConstraint} und repräsentiert eine Beziehung zwischen zwei \strong{Attributen} $x$ und $y$ verschiedener Views, die durch den Ausdruck
\begin{equation}
y = m*x + b
\end{equation}
gegeben ist. Dabei beschreiben $m$ und $b$ vom Typ \objcinline{float} \strong{Multiplier} und \strong{Constant} der Beziehung.
...
Die Auto Layout Syntax verfügt über die Attribute \objcinline{left, right, top, bottom, leading, trailing, width, height, centerX, centerY} und \objcinline{baseline}, wobei \objcinline{leading} und \objcinline{trailing} abhängig von der Leserichtung der eingestellten Sprache äquivalent oder umgekehrt zu \objcinline{left} und \objcinline{right} sind.
...
Bei der Konstruktion der Constraints sollte unbedingt auf ein eindeutiges Layout geachtet werden. Ein solches liegt vor, wenn jeder Subview ein eindeutiger Wert für alle vier Parameter des \objcinline{frame} Attributs zugewiesen werden kann.

\subsection{Intrinsic Content Size}

Subklassen von \objcinline{UIView} repräsentieren häufig Inhalt, dessen Darstellung eine bestimmte Größe erfordert. Dazu gehören bspw. \objcinline{UILabel} oder \objcinline{UIImageView} Objekte, deren Größe durch ihren Text- oder Bildinhalt bestimmt werden.

Diese \strong{Intrinsic Content Size} wird verwendet, um die Größe einer View zu berechnen. In den meisten Fällen sollte die Intrinsic Content Size nicht durch explizite \objcinline{width} und \objcinline{height} Constraints beschränkt werden.
...
Trotzdem können wir Constraints als Objekte der \objcinline{NSLayoutConstraint} Klasse im Code erstellen und einer View hinzufügen. Dazu implementiert \objcinline{UIView} die Instanzmethoden \objcinline{addConstraint:} und \objcinline{removeConstraint:} und gewährt über das Attribut \objcinline{constraints} Zugriff auf alle Constraints.

Einzelne Constraints können mit der Klassenmethode \objcinline{constraintWithItem:attribute:relatedBy:toItem:attribute:multiplier:constant:} instanziert werden. Da jedoch meist mehrere Constraints benötigt werden, stellt \objcinline{NSLayoutConstraint} zusätzlich eine Klassenmethode \objcinline{constraintsWithVisualFormat:options:metrics:views:} zur Verfügung, die eine \strong{Visual Format Language} verwendet.

Dabei übergeben wir der Klassenmethode einen String im ASCII-Art Stil, der die zu erstellenden Constraints beschreibt. So können wir bspw. einen Abstand von 10pt zwischen zwei Views \objcinline{view1} und \objcinline{view2} mit dem String
\begin{objccode}
@"[view1]-10-[view2]"
\end{objccode}
...
Diese Subklassen von \objcinline{UIViewController} sind der \strong{Controller}-Komponente des Model-View-Controller Konzepts zugeordnet \secref{sec:mvc}.

Jeder View Controller ist für die Verwaltung seiner eigenen \strong{Content View} zuständig. \objcinline{UIViewController} besitzt dafür ein Attribut \objcinline{view}.
...
Die oberste Instanz der View Hierarchie ist ein Objekt der \objcinline{UIWindow} Klasse. Analog besitzt \objcinline{UIWindow} ein Attribut \objcinline{rootViewController}, das die oberste Instanz der View Controller Hierarchie darstellt. Weisen wir diesem Attribut ein View Controller Objekt zu, so wird dessen Content View der View Hierarchie des \objcinline{UIWindow} Objekts hinzugefügt. Wird ein Storyboard verwendet, geschieht dies automatisch mit der Option \emph{Initial View Controller} im Attributes Inspektor des entsprechenden View Controllers.

Während nur in wenigen Fällen Subklassen von \objcinline{UIView} implementiert werden, wird man kaum eine iOS App ohne mindestens eine Subklasse von \objcinline{UIViewController} finden. Häufig stellt die Implementierung der View Controller den Großteil der Programmierung von iOS Apps dar.
...
\begin{itemize}
\item \objcinline{viewDidLoad} wird aufgerufen, sobald die Content View geladen wurde. An dieser Stelle können Attribute, die im Storyboard nicht hinreichend konfiguriert werden können, dynamisch angepasst werden, um einen Ausgangspunkt für die Präsentation zu schaffen.
\item \objcinline{viewWillAppear:}, \objcinline{viewDidAppear:}, \objcinline{viewWillDisappear:} und \objcinline{viewDidDisappear:} können verwendet werden, um die Inhalte der Content View zu aktualisieren oder Vorgänge zu beginnen/anzuhalten.
\item \objcinline{didReceiveMemoryWarning} wird aufgerufen, wenn das System die App auffordert, Speicher freizugeben. Hier sollten nicht mehr benötigte Objekte oder solche, die leicht wieder zu erstellen sind, freigegeben werden.
\end{itemize}

\subsection{Präsentation von View Controllern}

Die \objcinline{UIViewController} Klasse implementiert bereits eine einfache Möglichkeit, einen anderen View Controller temporär zu präsentieren. Die Instanzmethode \objcinline{presentViewController:animated:completion:} fügt die Content View des entsprechenden View Controllers der View Hierarchie hinzu, während \objcinline{dismissViewControllerAnimated:completion:} die Präsentation wieder beendet.
...
Das UIKit Framework stellt einige nützliche Subklassen von \objcinline{UIViewController} zu Verfügung, die in vielen Apps strukturgebend verwendet werden.

Insbesondere werden wir uns noch genauer mit dem äußerst vielseitigen \objcinline{UITableViewController} befassen. Dieser gehört mit \objcinline{UICollectionViewController} zu den inhaltsbasierten Subklassen von \objcinline{UIViewController}.

\objcinline{UINavigationController} und \objcinline{UITabBarController} hingegen sind als \strong{Container View Controller} konzipiert. Sie enthalten selbst nur wenige Subviews, bspw. in Form von Leisten. Stattdessen verwalten sie eine Hierarchie von weiteren View Controllern und präsentieren deren Content Views.

\subsubsection{Navigation Controller}

\objcinline{UINavigationController} implementiert eine \strong{Stack}\linkref{http://de.wikipedia.org/wiki/Stapelspeicher} Datenstruktur, die in Form eines Attributs \objcinline{NSArray *viewControllers} nach außen repräsentiert wird.

Einem Attribut \objcinline{UIViewController *rootViewController} kann zunächst ein View Controller zugewiesen werden, dessen Content View an erster Stelle der Hierarchie stehen soll. Anschließend kann dem Stack mit Aufrufen der Instanzmethode \objcinline{pushViewController:animated:} ein View Controller hinzugefügt und \objcinline{popViewControllerAnimated:} der oberste View Controller entfernt werden.

Jeder von einem Navigation Controller verwaltete View Controller kann über das Attribut \objcinline{UINavigationController *navigationController} auf diesen zugreifen.
...
Die in den Subviews der Content View des Navigation Controllers präsentierten Elemente, bspw. der Titel und die Buttons in Navigationsleiste und Toolbar, sind von dem jeweils präsentierten View Controller abhängig. Jeder View Controller besitzt dafür ein Attribut \objcinline{UINavigationItem *navigationItem}, das entsprechend konfiguriert werden kann und diese Informationen dem Navigation Controller bereitstellt.
...
Wird ein View Controller ausgewählt, können wir dessen Identität anpassen und die entsprechende Subklasse von \objcinline{UIViewController} auswählen, die wir implementiert haben \abbref{img:vc_identity}. Zur Laufzeit wird der View Controller dann als Objekt dieser Subklasse instanziert.
...
Anschließend stehen die im Code definierten IBOutlets und IBActions der \objcinline{UIViewController} Subklasse im Interface Builder zur Verfügung und können mit Interfaceelementen verbunden werden.
...
Ähnlich wie bei IBOutlets und IBActions innerhalb einer Scene gibt es mehrere Möglichkeiten, Segues zu erstellen. Dazu gehören bspw. das Ziehen einer Verbindungslinie mit gehaltener \keys{\ctrl}-Taste und die Verwendung des \emph{Triggered Segues} Abschnitts des Connection Inspectors.
...
Außerdem können Subklassen von \objcinline{UIControl} wie bspw. \objcinline{UIButton} bei einem \objcinline{UIControlEvent} Segues auslösen. So lassen sich Übergänge zwischen View Controllern bereits innerhalb des Storyboards erstellen.

\begin{itemize}
\item \strong{Modal Segues} sind das Äquivalent zum Aufruf der \objcinline{presentViewController:animated:completion:} Methode.
\item Befindet sich der View Controller des Ausgangspunkts der Segue in der View Hierarchie eines Navigation Controllers, kann eine \strong{Push Segue} als Äquivalent zu der \objcinline{pushViewController:animated:} Methode erstellt werden.
...
Dazu ist lediglich die Definition und (ggf. leere) Implementierung einer Methode mit Rückgabetyp \objcinline{IBAction} und genau einem Argument vom Typ \objcinline{UIStoryboardSegue} im Interface des View Controllers erforderlich. Der Methodenname sollte das Ziel der Unwind Segue beschreiben, also den implementierenden View Controller.

\begin{objccode}
- (IBAction)unwindToXYZ:(UIStoryboardSegue *)sender;
\end{objccode}

Die Unwind Segue ist anschließend im Storyboard in jedem View Controller in der Hierarchie des implementierenden View Controllers unter der \emph{Exit} Schaltfläche verfügbar und kann mit einem \objcinline{UIControlEvent} verbunden werden \abbref{img:unwind_segue}.
...
Im Code können wir auf das Auslösen einer Segue reagieren, um den View Controller des Ziels entsprechend zu konfigurieren. Dazu überschreiben wir die \objcinline{prepareForSegue:sender:} Instanzmethode und verwenden den Identifier:
...
Die aktive (fragende) Klasse stellt in ihrem öffentlichen Header ein \strong{Protokoll} zur Verfügung, das die verwendeten Methoden definiert. Außerdem besitzt sie ein öffentliches Attribut, häufig \objcinline{delegate} genannt, das eine Referenz zu einem Objekt der passiven (antwortenden) Klasse hält. Diese implementiert die Methoden des Protokolls, die in entsprechenden Situationen von der aktiven Klasse aufgerufen werden können.
...
Viele Klassen in Frameworks implementieren Kommunikationsmechanismen in Form von Delegates. Dazu gehört bspw. die im folgenden Abschnitt beschriebene \objcinline{UITableView}. Wir können jedoch auch eigene Protokolle und Delegates definieren. Betrachten wir dazu das Beispiel eines View Controllers \objcinline{SelectionViewController : UIViewController}, der zur Auswahl eines bestimmten Objekts dienen soll.
...
Das Delegate Attribut ist mit \objcinline{weak} gekennzeichnet, da die Kommunikation einseitig und die Referenz nicht notwendig ist: Die aktive Klasse soll das Delegate Objekt nicht im Speicher halten, wenn es an anderer Stelle nicht mehr benötig wird.

Weiterhin ist das Delegate Attribut als Empfänger des \objcinline{SelectionProtocol} markiert. Dem Attribut sollen also nur Objekte zugewiesen werden, die das Protokoll implementieren. Andernfalls wird eine Warnung ausgegeben.

Mit \objcinline{@protocol} beginnt schließlich die Definition des Protokolls. Die folgenden Methoden müssen vom Delegate implementiert werden. Optionale Methoden, deren Implementierung nicht erwartet wird, können einem \objcinline{@optional} folgen.
...
Aufgrund des eingeschränkten Platzes auf Geräten der iOS Plattform verwenden sehr viele Apps \objcinline{UIScrollView} oder Subklassen davon, um Views über die Bildschirmgröße hinaus darzustellen. \objcinline{UITableView} ist eine solche Subklasse, die in einem Großteil der iOS Apps zu finden ist.

\strong{Table Views} stellen eine Liste von \objcinline{UITableViewCell : UIView} Objekten dar und verwenden den Scrollmechanismus von \objcinline{UIScrollView}.

Eine Table View ist zunächst in \strong{Sections} unterteilt, die jeweils eine bestimmte Anzahl von \strong{Rows} enthalten. Zur Identifikation einer Zeile werden daher Objekte der \objcinline{NSIndexPath} Klasse verwendet, die jeweils ein Attribut \objcinline{NSInteger section} und \objcinline{NSInteger row} besitzen.

Es kann zwischen den Darstellungsoptionen \objcinline{UITableViewStylePlain} und \objcinline{UITableViewStyleGrouped} für Table Views gewählt werden, die lediglich das Erscheinungsbild der Table View verändern \abbref{img:tableview_styles}.
...
Statische Table Views bieten eine einfache Möglichkeit, Views tabellarisch darzustellen. Im Storyboard kann ein \objcinline{UITableView} Objekt der View Hierarchie hinzugefügt und mit der Option \emph{Content: Static Cells} versehen werden. Im Attributes Inspector kann die Anzahl der Sections und Rows, sowie eine Vielzahl weiterer Attribute konfiguriert werden. Die \objcinline{UITableViewCell} Objekte, die jede Zeile repräsentieren, fungieren nun als normale Views, denen wir Subviews in Form von Buttons, Labels o.ä. hinzufügen können.

Häufig möchten wir Table Views jedoch zur Darstellung dynamischer Inhalte verwenden. Dazu stellt \objcinline{UITableView} nach dem Delegate Konzept die beiden Protokolle \objcinline{UITableViewDatasource} und \objcinline{UITableViewDelegate} zur Verfügung und besitzt die entsprechenden Attribute \objcinline{id <UITableViewDatasource> datasource} und \objcinline{id <UITableViewDelegate> delegate}. Zur Laufzeit stellt eine dynamische Table View Anfragen an beide Objekte und benachrichtigt diese über Ereignisse.

Anstatt jede \objcinline{UITableViewCell} im Storyboard einzeln zu konfigurieren verwenden dynamische Table Views \strong{Prototype Cells}. Diese werden im Storyboard wie Static Cells konfiguriert, jedoch erst zur Laufzeit mit Inhalt gefüllt \abbref{img:tableview_prototypecells}. Natürlich eignet sich auch an dieser Stelle Auto Layout zur Positionierung der Views. Außerdem sollte jeder Prototype Cell ein Identifier zugewiesen werden.
...
An der entsprechenden Stelle im Code können wir dann mit der Instanzmethode \objcinline{dequeueReusableCellWithIdentifier:} von \objcinline{UITableView} ein \objcinline{UITableViewCell} Objekt nach Vorlage der Prototype Cell mit dem angegebenen Identifier anfordern und anschließend mit Inhalt füllen. Dabei verwendet UITableView einen sehr effektiven Verwaltungsmechanismus, der zunächst solche \objcinline{UITableViewCell} Objekte wiederverwendet, die gerade nicht angezeigt werden. So kann ein reibungsloses Scrollen durch beliebig lange Listen realisiert werden.
...
Prinzipiell kann die Table View an einer beliebigen Stelle der View Hierarchie der Content View eines View Controllers positioniert sein. Wurde ihr der View Controller als Datasource und Delegate zu gewiesen, bspw. mithilfe von IBOutlets im Storyboard, muss dieser die \objcinline{UITableViewDatasource} und \objcinline{UITableViewDelegate} Protokolle implementieren.

\begin{objccode}
@interface ViewController : UIViewController <UITableViewDatasource, UITableViewDelegate>
\end{objccode}

In den meisten Fällen verwenden wir jedoch die Table View selbst als Content View des View Controllers. UIKit stellt hierzu die vereinfachende Subklasse \objcinline{UITableViewController : UIViewController} zur Verfügung, die das Content View Attribut \objcinline{UIView *view} mit \objcinline{UITableView *tableView} ersetzt und darüber hinaus einige hilfreiche Mechanismen implementiert.

In der Implementierung der \objcinline{UITableViewController} Subklasse können wir nun die Methoden der Datasource und Delegate Protokolle implementieren. Die Dokumentation beschreibt die Protokolle in gewohnt ausführlicher Form.

Folgende Methoden sollten jedoch in keiner \objcinline{UITableViewDatasource} Implementierung fehlen:
...
Das \objcinline{UITableViewDelegate} Protokolls bietet zusätzlich zu vielen weiteren Darstellungsoptionen die Möglichkeit zur Reaktion auf verschiedene Ereignisse. Dazu gehört die vierte essentielle Methode der Table View Programmierung:
...
Alternativ zur Implementierung dieser Delegate Methode können auch Storyboard Segues in Verbindung mit Prototype Cells verwendet werden. In diesem Fall ist es sinnvoll, zunächst den Index Path der betätigten Table View Cell in der \objcinline{prepareForSegue:sender:} Methode auszulesen und damit bspw. Zugriff auf das entsprechende Model Objekt zu erhalten:
...
Häufig benötigen Apps lediglich die Speicherung einiger Parameter, Einstellungen und Benutzereingaben. Einfache Benutzerdaten können mithilfe der \objcinline{NSUserDefaults} Klasse gespeichert und über die Ausführung der App hinaus zu einem späteren Zeitpunkt wieder aufgerufen werden.

\objcinline{NSUserDefaults} implementiert die Klassenmethode \objcinline{standardUserDefaults}, die immer das gleiche Objekt zurückgibt (sog. Singleton Architektur). Die Klasse ähnelt einem \objcinline{NSDictionary} und implementiert analog nach dem Key-Value Prinzip die Instanzmethoden \objcinline{setObject:forKey:} und \objcinline{objectForKey:}.
...
Es können nur Objekte der Klassen \objcinline{NSString, NSNumber, NSDate} und \objcinline{NSData} oder ausschließlich mit solchen Objekten gefüllte Instanzen von \objcinline{NSArray} und \objcinline{NSDictionary} in dieser Form gespeichert werden.

Eigene Subklassen müssen daher zunächst zu \objcinline{NSData} Objekten serialisiert werden. Wird dies notwendig, sollte jedoch bereits über die Integration von Core Data nachgedacht werden.

Für einige Klassen wie \objcinline{UIImage} stehen Methoden zur Serialisierung zur Verfügung:
...
Elemente der Benutzeroberfläche werden hier anhand des \objcinline{NSString* restorationIdentifier} Attributs identifiziert. Bei der Wiederherstellung wird für jeden dieser Identifier ein entsprechendes Objekt angefordert, oder bei Verwendung eines Storyboards automatisch erstellt. View Controller und Views können dann die \objcinline{encodeRestorableStateWithCoder:} und \objcinline{decodeRestorableStateWithCoder:} Methoden implementieren, um ihre Darstellung zu archivieren und wiederherzustellen. Im iOS App Programming Guide \linkref{https://developer.apple.com/library/ios/documentation/iPhone/Conceptual/iPhoneOSProgrammingGuide/} ist das State Preservation System ausführlich dokumentiert.
...
Dazu implementiert das Foundation Framework die Klasse \objcinline{NSNotificationCenter} mit der Klassenmethode \objcinline{defaultCenter} nach dem Singleton Prinzip. Objekte können die Instanzmethode \objcinline{addObserver:selector:name:object:} nutzen, um bestimmte Nachrichten zu empfangen, oder mit der Instanzmethode \objcinline{postNotification:} eigene Nachrichten senden.
...
Meist sind die Testmethoden in Subklassen einer bestimmten Klasse des entsprechenden Unit Test Frameworks organisiert. In Objective-C steht dazu das \emph{XCTest} Framework \linkref{https://developer.apple.com/library/ios/documentation/ToolsLanguages/Conceptual/Xcode_Overview/UnitTestYourApp/UnitTestYourApp.html} mit der Klasse \objcinline{XCTestCase} zur Verfügung. Jede Testmethode ist durch den Prefix \objcinline{test} des Methodennamens gekennzeichnet und erscheint damit bspw. automatisch im Xcode \emph{Test Navigator} \secref{sec:navigator} und wird bei Betätigung von \keys{\cmd + U} automatisch ausgeführt. Außerdem werden die beiden Methoden \objcinline{setUp} und \objcinline{tearDown} der entsprechenden \objcinline{XCTestCase} Subklasse vor bzw. nach dem Test ausgeführt.

Ein Beispiel für einen Unit Test ist folgende Methode in einer \objcinline{XCTestCase} Subklasse:
...
Ich verwende bspw. in fast jedem meiner Projekte ein Repository \linkref{https://github.com/viWiD/VIOSFramework} als Submodule, in dem viele meiner häufig verwendeten Klassen ausgelagert sind. Dazu gehören bspw. Klassen wie \objcinline{VIArrayDataSource} \linkref{https://github.com/viWiD/VIOSFramework/tree/master/Data\%20Sources/VIArrayDataSource}, die eine Implementierung des \objcinline{UITableViewDataSource} Protokolls für Arrays darstellt und einfach wiederverwendet werden kann oder \objcinline{VILogger} \linkref{https://github.com/viWiD/VIOSFramework/tree/master/VILogger} als Ersatz für \objcinline{NSLog()}.
\end{itemize}
...
Nach dem Kompilieren erscheint das Archive im Organizer, auch aufrufbar mit der Tastenkombination \keys{\cmd+\shift+2}. Ein Klick auf \emph{Distribute} öffnet den Dialog zum Upload zu iTunes Connect. Hier kann das entsprechende Distribution Provisioning Profile und der korrespondierende Eintrag in iTunes Connect ausgewählt werden. Letzterer erscheint nur, wenn die App im iTunes Connect Interface zuvor mit einem Klick auf \emph{Ready to upload binary} markiert wurde. Anschließend wird die App Binary auf Konfigurationsfehler geprüft und hochgeladen.
