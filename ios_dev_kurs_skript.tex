...
\strong{Tipp:} Ist im Code an einer Stelle der Ausdruck \swift{// MARK: Section Title} zu finden, so erscheint \emph{Section Title} als Gliederung in dem Menü der Jump bar. Ein Gedankenstrich erzeugt eine horizontale Line: \objc{// MARK: - Section Title}.
...
\begin{swiftlst}
^\codehighlight{@IBOutlet}^ var label: UILabel! // Dieses Attribut ist als IBOutlet gekennzeichnet.
\end{swiftlst}
...
Einige Objekte stellen sog. \strong{Events} zur Verfügung, die in bestimmten Situationen ausgelöst werden. Dazu gehören bspw. Subklassen von \swift{UIControl}, also u.a. \swift{UIButton}, die das Event \emph{Touch Up Inside} auslösen, wenn der Benutzer seinen Finger im Bereich des Objekts anhebt.

Diese Events können mit Methoden im Code verbunden werden, die mit \swift{@IBAction} gekennzeichnet sind \abbref{img:xcode_ibaction}:

\begin{swiftlst}
^\codehighlight{@IBAction}^ func buttonPressed(sender: UIButton) { } // Diese Methode ist als IBAction gekennzeichnet.
\end{swiftlst}

\includegraphicsc{img/xcode_ibaction.png}{img:xcode_ibaction}{IBActions verbinden Events mit Methoden im Code}

Die verbundene Methode wird dann ausgeführt, wenn das entsprechende Event ausgelöst wird. Das auslösende Objekt wird der Methode als Parameter \objc{sender} übergeben.
...
\item Öffnet in Xcode den \strong{Devices} Bereich mit \keys{\cmdkey + \shiftkey + 2} und wählt in der linken Seitenleiste euer Gerät aus \abbref{img:xcode_device}.
