...
Der \strong{Navigator} (blau) dient zur Übersicht über die Projektelemente. Es kann zwischen acht Tabs (sieben vor Xcode Version 5.0) gewählt werden, die über die Tastenkombinationen \keysc{\cmdkey + 1}  bis \keysc{\cmdkey + 8} erreichbar sind:
\begin{enumerate}
...
Mit der \emph{Open Quickly} Funktion \keysc{\cmdkey + \shiftkey + O} kann ebenfalls schnell navigiert werden. Es öffnet sich ein Eingabefeld, mit dem auf die gesamte Indexierung des Projekts zugegriffen werden kann.
...
\item Öffnet in Xcode den \strong{Organizer} mit \keysc{\cmdkey + \shiftkey + 2}.
\item Wählt dort den Tab \menuc{Devices} und in der linken Seitenleiste euer Gerät aus \abbref{img:xcode_device}.
\item Mit den Schaltflächen \menuc{Use for Development} oder \menuc{Add to Member Center}) können nur Admins des Developer Teams ihre Geräte direkt im Team registrieren. Stattdessen kopiert bitte die UDID eures iOS Geräts, also die lange Zeichenfolge mit dem Label \emph{Identifier}.
...
Im Allgemeinen verwenden wir Methoden, um mit bestimmten Objekten zu interagieren, sog. \strong{Instanzmethoden}. In deren Implementierung kann dann auch auf Attribute des entsprechenden Objekts zugegriffen werden. Instanzmethoden werden mit dem Zeichen \objc{-} vor der Deklarierung gekennzeichnet:
\begin{objclst}
- (void)doSomething;
\end{objclst}
...
Mit Xcode kann ein \strong{Archive} kompiliert werden, das eine abgeschlossene Version der App darstellt. Wählt dazu \emph{iOS Gerät} als Target und anschließend \menuc{Product > Archive}.
